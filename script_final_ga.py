...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename='pure_ga.log', level=logging.INFO) # Logging name was changed accordingly for Hybrid and Plain GA runs.
    ...
    last_layer_key = list(state_dict.keys())[-2]  # Assuming the last key corresponds to the last layer Last Layer is head.bias , hence using -2 to access head.weight
    ...
    ga = GeneticAlgorithmEvolution(
        ...
        validation_class=validator,  # Pass the Model Validator instance
    )

    # Run the Genetic Algorithm Class
    best_weights = ga.run(population_size=50, num_generations=150, stop_threshold=98, save_interval=4)
    ...
    model_file = "experiments/model_ga_gen150_pop50.pt"
